...
def _execute_tool_internal(tool_name: str, tool_args: Dict[str, Any]) -> str:
    ...
    The 59-branch if/elif chain this used to be is now a table of handlers in
    blue/server/tool_handlers.py. A handler returning None means "not handled"
    — an unavailable subsystem, or a branch that fell off the end of its body
    — which the chain expressed by running on to the bottom.
    """
    handler = _tool_handlers.HANDLERS.get(tool_name)
...
def process_with_tools(messages: List[Dict], _pre_selection=None, user_name: str = "Alex", voice: bool = False, robot: str = "blue", language: str = "", focus: Optional[Dict] = None, system_addendum: str = "", on_token=None) -> Dict:
    ...
    # The fast path now lives in blue/server/tool_pipeline.py. It returns a
    # finished response, or None to fall through to the loop below.
...
from blue.server import tool_pipeline as _tool_pipeline
from blue.server import turn_context as _turn_context
from blue.server import turn_completion as _turn_completion
from blue.server import tool_handlers as _tool_handlers
...
def chat_completions():
    ...
    try:
        ...
        if not _is_zero_llm:
            # Sanitising, memory, dated recall, family facts, self-history,
            # camera memory, research and Wikipedia — see
            # blue/server/turn_context.py.
            messages = _turn_context.build(
                messages,
                _grounded_reply=_grounded_reply,
                _self_request_kind=_self_request_kind,
                focus=focus,
                language=language,
        ...
        if response:
            # Detection, the output guards, styling, speech and persistence —
            # see blue/server/turn_completion.py.
            response = _turn_completion.finish(
                response,
                _grounded_reply=_grounded_reply,
                last_user_msg=last_user_msg,
                messages=messages,
