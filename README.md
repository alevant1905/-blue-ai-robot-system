...
- Tick **continuous discussion** and the three of them talk it out on their own: a running order that keeps nobody twice in a row and nobody left out, with each turn answering the topic and the point the last speaker actually made. **Opens with** names who starts, or leaves it to the order. Say something at any moment — your line cuts the current turn short, joins the transcript, and the discussion carries on from it. **Pause discussion** holds them between turns without losing the running order (you can still ask them things while it is held); "Stop talking" ends it, and ticking the box again starts a fresh one.
- Add a website or PDF **in the middle of a discussion** and the next robot to speak has actually read it: any turn already prepared against the old state is thrown away, and that one turn is told to say what the document claims and whether it supports or undercuts the position being argued. After that it is simply part of what the panel is working from. **Pause between turns** sets the gap from 0 to 8 seconds and can be dragged while they are talking — the change lands on the very next pause. The next robot's turn is prepared *while* the current one is speaking, so replies land on the beat instead of leaving a generation-long hole; a turn prepared against a conversation Alex has since interrupted is thrown away rather than spoken.
